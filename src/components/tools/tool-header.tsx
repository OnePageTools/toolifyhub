...
export function ToolHeader({ title, description, icon, category }: ToolHeaderProps) {
  return (
    <header className="flex flex-col items-center text-center space-y-4 mb-8 md:mb-12 relative pt-8 px-4 md:px-0">
      ...
      {/* Icon Container */}
      <div className="w-[56px] h-[56px] rounded-[14px] bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center shadow-[0_8px_24px_rgba(59,130,246,0.3)] text-white">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-[15px] md:text-base max-w-xl mx-auto leading-relaxed">
          {description}
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
        {[
          ...
          <div key={pill.label} className={cn(
            "flex items-center gap-1.5 px-3 py-1 rounded-full border border-border text-[10px] font-black uppercase tracking-widest shadow-[0_1px_4px_rgba(0,0,0,0.06)] dark:shadow-none",
            pill.bg, pill.color
          )}>
