...
2.  **Generate Summary**: Based on the 'length' parameter, create a summary. Ensure the summary is significantly shorter than the original.
...
3.  **Respond in JSON**: Provide your response in the required JSON format. Do not include word counts in the JSON output.
...
const summarizeContentFlow = ai.defineFlow(
  ...
  async (input) => {
    try {
      ...
      const summaryWordCount = output?.summary.trim().split(/\s+/).length || 0;
